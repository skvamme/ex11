...
win(File) ->
    ...
    Text    = swColorText:make(Win, 10,10,XX,YY,1,?white),
    ...
    State = #e{text=Text,
	       width=XX,ht=YY,start=1,current=1,col=1,data=Lines},
...
display(#e{text=Text,width=W,ht=H,start=Start,current=Current,
    ...
    io:format("H=~p Len = ~p~n",[H, length(Strs)]),
    foldl(fun(Str,I) ->
		  Text ! {display, 1, I, normal, Str},
		  Len = length(Str),
		  Delete = W - Len,
		  if Delete > 0 ->
			  Del = duplicate(Delete, $\s),
			  Text ! {display, Len+1, I, normal, Del};
		     true -> void
		  end,
		  I+1
	  end, 1, Strs),
